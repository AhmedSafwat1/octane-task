...
export class BookController {
    ...
    @ApiOperation({ summary: 'Submit a reading interval for a book by authenticated user' })
    ...
    async submitIntervalByAuthUser(@Body() submitIntervalDto: SubmitIntervalAuthDto, @Request() req) {
    ...
    @Roles('admin')
    async storeBookByAdminUser(@Body() storeBookDto: StoreBookDto, @Request() req) {
    ...
    @UseGuards(AuthGuard('jwt'), RolesGuard)
    @Roles('admin')
    async updateBookByAdminUser(@Param('id') bookId: number, @Body() updateBookDto: StoreBookDto) {
